...
public abstract class FishEnchantmentRenderer {
	...
	public <R extends LivingEntityRenderState & GeoRenderState> void render(
		R state, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay, int color, RecursiveRenderer<R> recursiveRenderer) {

		RenderType renderLayer = this.model.getRenderType(state, this.model.getTextureResource(state));
	...
	public static class ExtraModel extends LectorfinEntityRenderer.LectorfinEntityModel {
		...
		public ExtraModel(ResourceLocation id) {
			super(id);
		}
